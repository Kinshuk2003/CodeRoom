export const handleTerminalConnection = (container, ws, projectId) => {
    console.log("Terminal Container id", container.id);

    // Execute default commands before allowing user to start giving commands
    container.exec({
        Cmd: ['sh', '-c', `cd $(ls -d */ | head -n 1) && npm install`],
        AttachStdout: true,
        AttachStderr: true,
        Tty: true,
        User: "sandbox",
        ...
        exec.start({ hijack: true, stdin: true }, (err, stream) => {
            ...
            // Show logs of the default commands to the user
            stream.on('data', (data) => {
            ...
            stream.on('end', () => {
                ...
                // Now connect the terminal from the frontend to the Docker container
                container.exec({
                    Cmd: ["/bin/bash"],
                    AttachStdin: true,
                    AttachStdout: true,
